...
Engine::Window::Window(const WindowConfig& cfg) : config(cfg) {
    ...
    monitor = config.fullscreen ? glfwGetPrimaryMonitor() : nullptr; // return monitor object if true
    mode = glfwGetVideoMode(glfwGetPrimaryMonitor()); // returns video mode of primary monitor to the video mode object
    ...
    window = glfwCreateWindow(
        config.fullscreen ? mode->width : config.width,
...
void Engine::Window::window_close_callback(GLFWwindow* window) {
    std::cout << "User has clicked close button\n";
}
...
void Engine::Window::setIcon(const std::string& imagePath) {
    ...
    unsigned char* pixels = stbi_load(imagePath.c_str(), &image.width, &image.height, &channels, 4);
    if (!pixels) {
        std::cerr << "Failed to load icon image: " << imagePath << std::endl;
        return;
    }
